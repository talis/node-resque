...
var failureBackend = require(__dirname + "/failure_backend.js").failureBackend;
...
var worker = function(options, jobs){
  ...
  self.failureBackendObject = new failureBackend({failureBackend:self.options.failureBackendImplementation});
...
worker.prototype.connect = function(callback){
  ...
  self.queueObject.connect(function(){
    ...
    self.checkQueues(function(){
      if (self.options.multipleFailureQueues) {
        for (queueName in self.queues) {
          self.connection.redis.sadd('failed_queues', queueName);
        }
      }
...
worker.prototype.fail = function(err, job) {
  var self = this;
  self.connection.redis.incr(self.connection.key('stat', self.failedQueueName(self.queue)));
  self.connection.redis.incr(self.connection.key('stat', self.failedQueueName(self.queue), self.name));
  self.connection.redis.rpush(self.connection.key(self.failedQueueName(self.queue)), JSON.stringify(self.failurePayload(err, job)));
  self.emit('failure', self.queue, job, err);
};

worker.prototype.failedQueueName = function(queue) {
  //return this.failureBackendObject.failedQueueName(queue);
  if (this.options.multipleFailureQueues) {
